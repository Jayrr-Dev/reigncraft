...
| **Procedural trees & rocks** | Feature flag (default **on**). When **off**, open-world tree and stone/column-rock resolvers return null; no Graphics bake, no chop/mine targets from procedural props. Placed player trees stay. Features panel toggle; `localStorage` key `world-plaza-procedural-trees-and-rocks-enabled`. |
...
| **SFX play helper**         | `playingWorldPlazaStarAudioSfx` — equipment impacts play on the shared SFX group (replaces raw `starAudio.play(..., { group: 'sfx' })` in the equipment hook). Optional `duration` (seconds) schedules a hard `stop()` because star-audio's own `play()` ignores duration. |
