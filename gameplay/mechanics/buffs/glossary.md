...
| **Companion modifiers** | Extra movement rows applied with same expiry (e.g. skybound jump arc + layer reach). |
...
| Term | Meaning |
| ---- | ------- |
| **Well-fed buff** | `well-fed-*` timed buffs rolled on cooked meat eat (`cookedWellFedChance`). |
| **Disease symptom buff** | `disease-*` debuffs granted by disease stage scheduler. Often `hideFromHud: true`. |
...
| **Disease HUD entry** | Separate row from symptomatic `diseaseEffects` (not a buff catalog id). |
| **Mechanics badge guide** | Home UI cards from `listingPlazaMechanicsBuffBadgeGuideEntries` (all non-hidden registry entries). |
| **Remaining seconds** | `computingWorldPlazaEntityBuffHudRemainingSeconds` for timed rows. |
