...
| **Pool max playback**   | Optional hard stop (seconds) per vocal pool via `DEFINING_WILDLIFE_FARM_ANIMAL_SFX_POOL_MAX_PLAYBACK_DURATION_S` → `playingWorldPlazaStarAudioSfx({ duration })`. `pig_grunt` caps at **2.2s**.                              |
| **Pool replay gap**     | Per-instance min interval before the same pool can fire again (`DEFINING_WILDLIFE_FARM_ANIMAL_SFX_POOL_MIN_REPLAY_INTERVAL_MS`). `pig_grunt` **8s**.                                                                          |
