...
| **Dev QA load**       | Home-screen **This a dev load** session. Compact all-biome grid, frozen wildlife AI, no aggro, manual spawner only. Store: `managingWorldPlazaDevQaLoadStore.ts`.                                                                                                               |
...
| **Size σ tier**                 | Discrete name-tag / age proxy bucket from the size sample: **−2** baby … **+3** legendary. Config: `DEFINING_WILDLIFE_NAME_TAG_TIER_CONFIG`.                                                                                                                                              |
| **Name tag**                    | Label above the sprite when revealed: `[prefix] species [suffix]`. Prefix pools are per size σ tier (e.g. +1σ is only Mama / Dada / Daddy / Mommy). Pack alpha overrides to **Alpha**.                                                                                                    |
...
| **Vocal concurrency**   | One active vocal per animal instance. Repeated or lower-priority events wait for the current clip to finish; combat vocals may interrupt idle calls. Different animals may vocalize together.                                                                                                                              |
...
| **Pool max playback**   | Optional hard stop (seconds) per vocal pool (`DEFINING_WILDLIFE_*_SFX_POOL_MAX_PLAYBACK_DURATION_S` → `playingWorldPlazaStarAudioSfx({ duration })`). Covers farm beds (`pig_grunt`, `sheep_baa`, `chicken_cluck`, `dog_bark`, `donkey_bray`, `goat_bleat`, `tiger_growl`, `wolf_howl`), Pixabay beds, and `beast_bellow`. |
| **Pool replay gap**     | Per-instance min interval before the same pool can fire again (`DEFINING_WILDLIFE_*_SFX_POOL_MIN_REPLAY_INTERVAL_MS`). Typical farm/Pixabay bed pools **7–8s**; wolf howl **14s**.                                                                                                                                         |
| **Omega-wolf pack**     | Separate werewolf clips (`public/creatures/sfx/vocals/werewolf/`); not merged into farm-animal pools.                                                                                                                                                                                                                      |
| **Wildlife footstep**   | Surface one-shot per walk/run stride on moving instances; `usingWildlifeFootsteps.ts`. Uses wildlife-only clip pools (`DEFINING_FILMCOW_FOOTSTEP_WILDLIFE_SURFACE_DEFINITIONS`), never avatar samples.                                                                                                                     |
| **Footstep size tier**  | `tiny`–`heavy` from visual size; drives clip override, volume scale, and playback-rate multiplier. Medium volume = **1/3** avatar footstep target (`DEFINING_FILMCOW_FOOTSTEP_WILDLIFE_VOLUME_RELATIVE_TO_AVATAR`).                                                                                                        |
| **Short one-shot rule** | Skips FilmCow composite run loops (`grass_run`, `leaves_run`, `dirt_run`); caps playback at **0.52s** walk / **0.28s** run (honored by `playingWorldPlazaStarAudioSfx` scheduled stop).                                                                                                                                    |
