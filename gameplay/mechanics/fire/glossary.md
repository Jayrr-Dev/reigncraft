...
| **Active tile key set**   | Engine-only: `activeFireTileKeysRef` in `renderingWorldPlazaFireLayer.tsx`. Cleared and refilled each safe tick to reconcile the flame visual pool. Not a player mechanic.                                             |
| **Loop-safe fire tick**   | Engine-only: fire layer Pixi frame uses `usingWorldPlazaSafeTick` (`tick:fire`). Tick errors log to the client debug buffer without stopping other plaza systems. Not a player mechanic.                               |
