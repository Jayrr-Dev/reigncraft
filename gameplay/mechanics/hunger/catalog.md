...
| Fish (raw catch)                      | `DEFINING_WORLD_PLAZA_HUNGER_RESTORE_FISH`        | 0.20          | 20%       |
| Wheat (harvested)                     | `DEFINING_WORLD_PLAZA_HUNGER_RESTORE_WHEAT`       | 0.18          | 18%       |
| Cooked meat (legacy generic constant) | `DEFINING_WORLD_PLAZA_HUNGER_RESTORE_COOKED_MEAT` | 0.60          | 60%       |

Species meat uses per-entry ratios in `definingWildlifeMeatRegistry.ts`. Fish and wheat item rows live in `definingWorldPlazaInventoryItemTypes.ts`. See [inventory-food catalog](../inventory-food/catalog.md).
...
| Berries / apple / fish / wheat restore | `definingWorldPlazaHungerConstants.ts` + `definingWorldPlazaInventoryItemTypes.ts` |
