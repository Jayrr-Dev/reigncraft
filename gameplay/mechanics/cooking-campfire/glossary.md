...
| **Fallback raw poison**    | If disease roll misses, generic toxic DoT (`DEFINING_WILDLIFE_RAW_MEAT_POISON_FLAT_EV` **5** over **60s**), stamped on the **simulation clock**.                               |
| **Food sickness**          | Hunger restore × **0.5** when this bite contracts disease or the player is already symptomatic ([disease](../disease/) + `DEFINING_WILDLIFE_FOOD_SICKNESS_HUNGER_MULTIPLIER`). |
| **World epoch (eat)**      | `worldEpochMs` / `Date.now()` for disease incubation scheduling on contract.                                                                                                   |
| **Simulation clock (eat)** | `nowMs` / `performance.now()` for well-fed buffs, poison DoT, and disease grant effect stamps.                                                                                 |
