...
Plaza scene passes **two clocks** into the eat resolver:

| Param          | Clock                            | Used for                                                                      |
| -------------- | -------------------------------- | ----------------------------------------------------------------------------- |
| `nowMs`        | Simulation (`performance.now()`) | Well-fed buff stamps, fallback poison DoT, disease grant effect `expiresAtMs` |
| `worldEpochMs` | Wall (`Date.now()`)              | Disease incubation / grant fire scheduling (survives logout)                  |

Unit tests may omit `worldEpochMs` (defaults to `nowMs`).

### Raw meat

1. Roll `sicknessRoll < rawDiseaseChance` → contract [disease](../disease/) with both clocks (`applyingWorldPlazaEntityDisease`).
2. On miss → fallback toxic poison DoT (EV **5** over **60s**), stamped on the simulation clock.
3. Hunger restore × **0.5** if this bite contracted disease or the player is already symptomatic (world-epoch check).

### Cooked meat

1. Roll `sicknessRoll < cookedResidualDiseaseChance` when residual fields exist (deer **5%** chronic-wasting, cow **3%** mad-cow).
2. Separate roll `wellFedRoll < cookedWellFedChance` → apply `well-fed-*` buff from registry on the simulation clock.
