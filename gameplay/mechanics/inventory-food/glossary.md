...
| **Eat channel**              | Timed interaction before effects apply; player is held in place until complete, damage cancel, or walk cancel.                                                             |
| **Eat duration**             | Channel length in ms (`1_000`–`10_000`) from `resolvingWorldPlazaInventoryFoodEatDurationMs`.                                                                              |
| **Eating overlay**           | World-anchored eating sound line (e.g. "Chomp ChomP CHOMP...", slow beat of three) + progress ring above the avatar while the channel runs.                                |
| **Damage cancel**            | New damage (`lastDamagedAtMs` after channel start) aborts the eat; item is not consumed.                                                                                   |
| **Walk cancel**              | Keyboard walk, click-walk target, jump, or roll aborts the eat; item is not consumed.                                                                                      |
...
| **Tiered tool registration**    | `registeringWorldPlazaTieredToolInventoryItems()` spreads sword/axe/pickaxe/hoe/scythe/fishrod rows (wood→gold) into the item type list. Legacy wood axe stays as inline `world-plaza-axe`. Wood pickaxe (`world-plaza-pickaxe`) registers with the other pickaxe tiers.                                                        |
| **iconImageUrl**                | Optional Vite-bundled PNG URL on an item type (from `?url` imports in `definingWorldPlazaToolInventoryIconConstants.ts`). Glyph renderer prefers this over `iconifyIcon` / Lucide / emoji. Pixel-art tools use the Tools Icons pack under `src/client/world/inventory/assets/tools-icons/` (mirrored in `public/tools-icons/`). |
| **iconifyIcon**                 | Optional bundled Iconify glyph id on an item type (e.g. `game-icons:scythe`, `mdi:hammer`). Used when no `iconImageUrl`. Must be registered in `registeringBundledIconifyIcons.ts`.                                                                                                                                             |
...
| **Weapon/tool slot**            | Far-left hotbar index **0** (`DEFINING_WORLD_PLAZA_INVENTORY_WEAPON_TOOL_SLOT_INDEX`). Only items with `equipment.toolKinds` may occupy it. Whatever is in this slot is always equipped. Empty shows a faded fist (unarmed). Charcoal border outlines the socket.                                                               |
...
| **Pickup channel**              | Timed hold before a ground stack enters inventory. Default duration **0.5–10 s** from weight (`resolvingWorldPlazaGroundItemPickupDurationMs`). Walk out of range cancels. Optional `durationMs` override on channel start.                                                                                                     |
| **Contested pickup**            | Pickup started while wildlife is eating that stack. Duration rolls **2–10 s** (`rollingWildlifeContestedGroundFoodPickupDurationMs`) instead of weight. Starting the channel hard-aggros the eater (meal theft; see [wildlife](../wildlife/)).                                                                                  |
