...
| `.cursor/hooks/check-gameplay-doc-drift.mjs` | **Disabled.** Opt-in only (`GAMEPLAY_DOC_DRIFT_HOOK=1` + re-register on `stop`). Prefer `/update docs` |
| `.cursor/rules/gameplay-docs.mdc`            | Agent rule: sync gameplay docs **only when the user asks** (`/update docs`)                            |

### Player-facing Guide companions

When you run `/update docs` (or otherwise ask for doc sync), also list related Guide surfaces (Controls, Mechanics, Biomes, Bestiary). Update those files if the change is player-visible there, or mark N/A if not.
