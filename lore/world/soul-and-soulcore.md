# The Soul and Soulcore

- **lore-id:** `soul-and-soulcore`
- **Canon status:** Established (Soulcore item, non-droppable stack behavior, Soulbreak damage, death screens); Proposed (Manus-canon meaning, drop-on-death and wealth-scent mechanics); Deprecated (the old folk theories, kept at the bottom)
- **Sources:** `definingWorldPlazaSoulcoreConstants.ts` (name, K/M/B/T stack display), `definingWorldPlazaInventoryItemDescriptionCorpus.ts` ("Condensed soul energy"), `definingWorldPlazaEntityDamageKindRegistry.ts` (Soulbreak)

## Soulcore

Soulcore is condensed soul energy: small purple orbs, warm to the touch, faintly humming. Everyone and everything on the ladder has a core, and everything drops it when it dies. A Soulcore is proof that you survived, made portable.
...
> Naming note for writers: the designer's notes call these **Magiccores**. The shipped game says **Soulcore**, so the bible uses Soulcore everywhere and treats it as carrying the full Magiccore meaning above. Final naming is an open question ([`meta/open-questions.md`](../meta/open-questions.md)).
...
> Status note: this whole section is Proposed. The shipped Soulcore is non-droppable and never lost on death (`definingWorldPlazaSoulcoreConstants.ts` and inventory behavior). The 20% drop and wealth-scent are designer intent that conflicts with shipped behavior, flagged in [`meta/open-questions.md`](../meta/open-questions.md). Do not put drop-on-death claims in shipped copy until the mechanic exists.
...
- ~~The residue theory (soul leakage condensing over ages)~~ and ~~the forge theory (Forgewrights condensed souls deliberately)~~: both written when Soulcore's origin was a mystery. It is not a mystery. Manus made the ladder and the cores keep its score.
...
- ~~Soulcore "cannot be dropped or thrown away" as a fiction rule.~~ True of shipped mechanics today, but the fiction is moving toward loot-on-death; see the open question before writing either version into shipped copy.

## Writing rules for soul content

- Soulcore is physical and mundane in handling: counted, hoarded, spent, stolen off corpses. The uncanny part is what it counts, not what it is.
