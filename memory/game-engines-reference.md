...
| **Perf knobs**      | `DEFINING_WORLD_PLAZA_PERFORMANCE_PROFILES`: floor/elevation build budgets, `treeBuildBudgetPerFrame`, prefetch rings, `visibleBoundsSnapTiles`, tier flags (`drawsTreeShadows`, `drawsTreeShake`, `drawsPlacedBlockShadowBlur`, `waterShimmerUpdateIntervalFrames`), `navigationMaxNodeExpansions`, `remoteAvatarPresentationCullGridRadius`, `wildlifePresentationCullGridRadius`. Idle heavy-skip key uses snapped `floorBoundsKey` (not every player tile). Feature flag **Procedural trees & rocks** (`managingWorldPlazaProceduralTreesAndRocksFeatureStore`) gates open-world tree/stone resolvers; Features debug panel toggle. |
