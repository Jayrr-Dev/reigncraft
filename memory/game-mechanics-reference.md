...
- Local sources: lava **920°C**, campfire tile **50°C** (comfort high, lit cells only), ice block **−22°C**, frozen water **−14°C**
...
**Wildlife footsteps:** Dedicated short one-shots (never avatar clip pools); medium volume **1/3** of player footsteps (`DEFINING_FILMCOW_FOOTSTEP_WILDLIFE_VOLUME_RELATIVE_TO_AVATAR`); max **4** steps/tick, **80 ms** poll; playback capped **0.52s** walk / **0.28s** run (`usingWildlifeFootsteps.ts`).
...
**Herbarium codex:** Guide → Herbarium; **12** flowers + **10** tree variants; flowers unlock on **pick** only (backfill from inventory + picked tiles); pickable petals = chromatic **or bright white** (green / dull grey skipped); larger outlined surface dots; wildflowers in plains / forest / flower forest / jungle / swamp / savanna (not beach, snow, desert, rocky, badlands, ocean, firelands); inventory **Study** consumes **1** and awards **+1** study; trees sight within **12** grid, study **+1** per chop layer; tiers at **1 / 5 / 15 / 25**. Progress localStorage-only (no cloud save mirror yet).
...
- Scatter: all biomes except Firelands/water; off-rocky noise min **0.72**; rocky pebbles **0.78** (columns stay **0.4**); on-tile jitter ±**20**/±**10** px; floor draw on all perf tiers (`drawsStoneDecorations`)
- Rocky pebble-field medium boulders: below large tier → **1×1** footprint, surface layer **4**; large tier keeps mega variety
- Rocky field climate: temperature **0.39 to 0.55**, humidity **0.29 to 0.41**; sample coverage about **4.2%**
