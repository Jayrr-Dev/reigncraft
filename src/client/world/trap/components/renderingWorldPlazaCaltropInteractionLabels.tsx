/**
 * DOM interaction labels for selected caltrops (Pick up).
 *
 * @module components/world/trap/components/renderingWorldPlazaCaltropInteractionLabels
 */

'use client';

import {
  DEFINING_WORLD_PLAZA_CALTROP_INTERACTION_LABEL_BUTTON_CLASS_NAME,
  DEFINING_WORLD_PLAZA_CALTROP_INTERACTION_LABEL_BUTTON_STACK_CLASS_NAME,
  LABELING_WORLD_PLAZA_CALTROP_PICK_UP_ACTION,
} from '@/components/world/trap/domains/definingWorldPlazaCaltropConstants';
import {
  listingWorldPlazaCaltropsInInteractionRange,
  type ListingWorldPlazaCaltropsInInteractionRangeAction,
  type ListingWorldPlazaCaltropsInInteractionRangeEntry,
} from '@/components/world/trap/domains/listingWorldPlazaCaltropsInInteractionRange';
import {
...
const RENDERING_WORLD_PLAZA_CALTROP_INTERACTION_LABEL_HIDDEN_TRANSFORM =
  'translate(-9999px, -9999px)' as const;

const RENDERING_WORLD_PLAZA_CALTROP_INTERACTION_LABEL_WRAPPER_CLASS_NAME =
  'pointer-events-none absolute left-0 top-0 z-10 will-change-transform select-none' as const;

export type RenderingWorldPlazaCaltropInteractionLabelsProps = {
...
/**
 * Pick up labels above selected caltrops.
 */
export function RenderingWorldPlazaCaltropInteractionLabels({
  selectedInteractableBlockKeysRef,
  cameraOffsetRef,
  cameraWorldZoomRef,
  onTrapAction,
}: RenderingWorldPlazaCaltropInteractionLabelsProps): React.JSX.Element {
  ...
  useLayoutEffect(() => {
    ...
    const updatingLabels = (): void => {
      ...
      const nextSelectedTraps = listingWorldPlazaCaltropsInInteractionRange(
    ...
    <div className="pointer-events-none absolute inset-0 overflow-visible">
      {selectedTraps.map((entry) => (
        <div
          ...
          className={
            RENDERING_WORLD_PLAZA_CALTROP_INTERACTION_LABEL_WRAPPER_CLASS_NAME
          }
          style={{
            transform:
              RENDERING_WORLD_PLAZA_CALTROP_INTERACTION_LABEL_HIDDEN_TRANSFORM,
          }}
        >
          <div
            ref={(element) => {
          ...
          >
            {entry.actions.map((action) => (
              <RenderingWorldPlazaTimedInteractionLabelRow
                ...
                label={LABELING_WORLD_PLAZA_CALTROP_PICK_UP_ACTION}
