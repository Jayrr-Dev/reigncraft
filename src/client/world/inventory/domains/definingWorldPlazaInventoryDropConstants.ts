...
export const DEFINING_WORLD_PLAZA_INVENTORY_DROP_PREVIEW_ARROW_BASE_PX = 28 as const;

/** Small lift above tile center so the arrow sits on the diamond, not below it. */
export const DEFINING_WORLD_PLAZA_INVENTORY_DROP_PREVIEW_ARROW_VERTICAL_OFFSET_PX = 8 as const;

/** Bob amplitude while the drop arrow is visible (pixels). */
export const DEFINING_WORLD_PLAZA_INVENTORY_DROP_PREVIEW_ARROW_BOB_AMPLITUDE_PX = 3 as const;

/** Bob cycle duration (ms). */
export const DEFINING_WORLD_PLAZA_INVENTORY_DROP_PREVIEW_ARROW_BOB_PERIOD_MS = 900 as const;
...
export const DEFINING_WORLD_PLAZA_INVENTORY_DROP_TILE_OUTLINE_Z_INDEX_OFFSET = 4 as const;

/** Root wrapper for the drop arrow DOM overlay. */
export const STYLING_WORLD_PLAZA_INVENTORY_DROP_ARROW_ROOT_CLASS_NAME =
  "pointer-events-none absolute left-0 top-0 z-[25] flex items-center justify-center" as const;
