/**
 * Viewport sizing for the action-bar temperature orb.
 *
 * @module components/world/health/domains/resolvingWorldPlazaTemperatureIndicatorViewportStyles
 */
...
/** Design-space font size for the temperature readout inside the orb. */
export const DEFINING_WORLD_PLAZA_TEMPERATURE_INDICATOR_VALUE_BASE_PX =
  9 as const;

export type DefiningWorldPlazaTemperatureIndicatorViewportStyles = {
  readonly sphereStyle: CSSProperties;
  readonly valueStyle: CSSProperties;
};

/**
 * Viewport styles that size the temperature sphere to match action bar buttons.
 *
 ...
 */
export function resolvingWorldPlazaTemperatureIndicatorViewportStyles(
  viewportHudScale: number,
  isMobile = false
): DefiningWorldPlazaTemperatureIndicatorViewportStyles {
  const designScale =
    DEFINING_WORLD_PLAZA_ACTION_BAR_SCALE *
    (isMobile ? DEFINING_WORLD_PLAZA_ACTION_BAR_MOBILE_SCALE : 1);

  const valueSizePx = computingWorldPlazaViewportHudScaledPx(
    DEFINING_WORLD_PLAZA_TEMPERATURE_INDICATOR_VALUE_BASE_PX,
    viewportHudScale,
    designScale
  );
