/**
 * Runs the shared statechart engine for PackHunter-temperament wildlife.
 *
 * @module components/world/wildlife/domains/advancingWildlifePackHunterBehaviour
 */
...
import { DEFINING_WILDLIFE_PACK_HUNTER_BEHAVIOUR_MACHINE } from '@/components/world/wildlife/domains/definingWildlifePackHunterBehaviourMachine';
import {
  DEFINING_WILDLIFE_PACK_HUNTER_BEHAVIOUR_REGISTRY,
  type DefiningWildlifePackHunterBehaviourContext,
} from '@/components/world/wildlife/domains/definingWildlifePackHunterBehaviourRegistry';
import {
...
export type AdvancingWildlifePackHunterBehaviourParams = {
...
function listingPackHunterBehaviourTimerEvents(
  context: DefiningWildlifePackHunterBehaviourContext
...
function resolvingPackHunterBehaviourSnapshot(
  aggroState: DefiningWildlifeAggroState
): DefiningStateMachineSnapshot {
  return {
    machineId: DEFINING_WILDLIFE_PACK_HUNTER_BEHAVIOUR_MACHINE.id,
    definitionVersion: DEFINING_WILDLIFE_PACK_HUNTER_BEHAVIOUR_MACHINE.version ?? 1,
...
function creatingPackHunterBehaviourContext(
  params: AdvancingWildlifePackHunterBehaviourParams,
  aggroState: DefiningWildlifeAggroState
): DefiningWildlifePackHunterBehaviourContext {
...
/**
 * Advances PackHunter behaviour through the shared statechart engine.
 * Replaces the bespoke phase tick; any `PackHunter` temperament species uses this.
 */
export function advancingWildlifePackHunterBehaviour({
  instance,
  species,
  nearbyInstances,
  playerPosition,
  ...
  tickEvents = [],
  resolveSpecies,
}: AdvancingWildlifePackHunterBehaviourParams): DefiningWildlifeAggroState {
  if (species.temperamentId !== 'pack_hunter') {
    return aggroState;
  }
  ...
  const context = creatingPackHunterBehaviourContext(
    {
      instance,
      species,
      nearbyInstances,
  ...
  const timerEvents =
    context.prey === null ? [] : listingPackHunterBehaviourTimerEvents(context);
  ...
  const result = processingStateMachineExplicitEvents({
    definition: DEFINING_WILDLIFE_PACK_HUNTER_BEHAVIOUR_MACHINE,
    registry: DEFINING_WILDLIFE_PACK_HUNTER_BEHAVIOUR_REGISTRY,
    snapshot: resolvingPackHunterBehaviourSnapshot(nextAggroState),
    events,
    context,
