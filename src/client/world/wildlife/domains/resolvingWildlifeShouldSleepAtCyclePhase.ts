...
function checkingWildlifeIsNightPhase(cyclePhase: number): boolean {
...
export function resolvingWildlifeShouldSleepAtCyclePhase({
  ...
  if (activityPattern === 'crepuscular') {
    return (
      checkingWildlifeIsNightPhase(cyclePhase) &&
      !checkingWildlifeIsCrepuscularActivePhase(
        cyclePhase,
        sleepProfile.phaseWindowOffset
      )
    );
  ...
  return (
    checkingWildlifeIsNightPhase(cyclePhase) &&
    checkingWildlifeCathemeralShouldSleep(
      instanceId,
      cyclePhase,
