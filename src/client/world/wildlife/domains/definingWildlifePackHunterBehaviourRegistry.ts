/**
 * PackHunter behaviour domain registry: guards and actions for the shared
 * statechart engine. Any species with temperamentId `pack_hunter` uses this
 * vocabulary; species-specific tuning stays in constants and species defs.
 *
 * @module components/world/wildlife/domains/definingWildlifePackHunterBehaviourRegistry
 */
...
export type DefiningWildlifePackHunterBehaviourContext = {
...
export type DefiningWildlifePackHunterBehaviourCommand = never;

function releasingPackHunterAggroOnPrey(
...
/** PackHunter bounded-context registry for the shared statechart engine. */
export const DEFINING_WILDLIFE_PACK_HUNTER_BEHAVIOUR_REGISTRY: DefiningStateMachineRegistry<
  DefiningWildlifePackHunterBehaviourContext,
  DefiningWildlifePackHunterBehaviourCommand
> = {
  guards: {},

  actions: {
    'PackHunter.commitAttack': (context) => {
      context.aggroState = {
        ...context.aggroState,
    ...
    'PackHunter.applyEnrageThreat': (context, event) => {
      if (!context.preyTargetId) {
        return;
      }
    ...
    'PackHunter.onEnterIdle': (context, event) => {
      ...
      if (shouldReleaseAggro) {
        releasingPackHunterAggroOnPrey(context);
      }
    ...
    'PackHunter.onEnterShadowing': (context) => {
      context.aggroState = {
        ...context.aggroState,
        stalkAttackingPreySinceMs: null,
        stalkPlayerApproachState: null,
    ...
    'PackHunter.onEnterRetreating': (context) => {
      context.aggroState = {
        ...context.aggroState,
        stalkAttackingPreySinceMs: null,
      };
    ...
    'PackHunter.onEnterFleeing': (context, event) => {
      const releasedAggro =
    ...
    'PackHunter.onEnterRegrouping': (context) => {
      context.aggroState = {
        ...context.aggroState,
        stalkAttackingPreySinceMs: null,
        stalkPlayerApproachReactedAtMs:
    ...
    'PackHunter.onEnterAttacking': (context, event) => {
      if (
    ...
    'PackHunter.onEnterFormingUp': () => {},
    'PackHunter.onEnterSurrounding': (context) => {
