...
/** Lift above the flower dot for the interaction popover anchor (world-local px). */
export const RESOLVING_WORLD_PLAZA_FLOWER_INTERACTION_LABEL_OFFSET_ABOVE_PX = 18;

/**
 * Maps a biome flower tile to viewport coordinates for its pick popover.
 */
export function resolvingWorldPlazaFlowerInteractionLabelScreenPoint(
  tileX: number,
  tileY: number,
  ...
  return {
    ...
    y:
      viewportPoint.y -
      RESOLVING_WORLD_PLAZA_FLOWER_INTERACTION_LABEL_OFFSET_ABOVE_PX *
        cameraWorldZoom,
  };
}
