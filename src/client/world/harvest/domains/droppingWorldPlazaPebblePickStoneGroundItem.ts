...
export const DEFINING_WORLD_PLAZA_PEBBLE_PICK_GROUND_DROP_SLOT_INDEX = -1;

export type DroppingWorldPlazaPebblePickStoneGroundItemParams = {
...
export type DroppingWorldPlazaPebblePickStoneGroundItemResult =
  | {
...
/**
 * Spawns stone from a completed pebble pick as a ground item at the pebble tile.
 */
export async function droppingWorldPlazaPebblePickStoneGroundItem({
  ...
  stoneQuantity,
  playerPosition,
}: DroppingWorldPlazaPebblePickStoneGroundItemParams): Promise<DroppingWorldPlazaPebblePickStoneGroundItemResult> {
  if (stoneQuantity <= 0) {
  ...
  const dropRequest = {
    ...
    gridY: tileY,
    layer,
    slotIndex: DEFINING_WORLD_PLAZA_PEBBLE_PICK_GROUND_DROP_SLOT_INDEX,
