...
function drawingWorldPlazaLakeWaterRippleRingOnGraphics(
  ...
  graphics
    .ellipse(
      ...
      DEFINING_WORLD_PLAZA_WATER_LAKE_RIPPLE_RADIUS_Y_PX + expandOffsetPx * 0.4
    )
    .stroke({
...
function drawingWorldPlazaLakeWaterShineOnGraphics(
  ...
  const phase =
    ((animationTimeMs +
      shineSeed * DEFINING_WORLD_PLAZA_WATER_LAKE_SHINE_PERIOD_MS) %
      DEFINING_WORLD_PLAZA_WATER_LAKE_SHINE_PERIOD_MS) /
    DEFINING_WORLD_PLAZA_WATER_LAKE_SHINE_PERIOD_MS;
...
export function drawingWorldPlazaLakeWaterRipplesAndShineOnGraphics(
  ...
  const drawsRipple =
    rippleSeed > DEFINING_WORLD_PLAZA_WATER_LAKE_RIPPLE_TILE_THRESHOLD;
  const drawsShine =
    shineSeed > DEFINING_WORLD_PLAZA_WATER_LAKE_SHINE_TILE_THRESHOLD;

  if (!drawsRipple && !drawsShine) {
    return;
  }
