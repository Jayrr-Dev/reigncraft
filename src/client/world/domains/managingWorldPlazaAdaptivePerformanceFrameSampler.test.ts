...
import {
  resolvingWorldPlazaAdaptivePerformanceTierCeiling,
  resolvingWorldPlazaInitialPerformanceTier,
} from '@/components/world/domains/resolvingWorldPlazaPerformanceProfile';
...
function fillingSamplerHistory(
  sampler: ReturnType<typeof creatingWorldPlazaAdaptivePerformanceFrameSampler>,
  frameDeltaMs: number,
  frameCount: number,
  startNowMs: number
...
describe('resolvingWorldPlazaAdaptivePerformanceTierCeiling', () => {
  it('caps LOW starters at LOW', () => {
    expect(
      resolvingWorldPlazaAdaptivePerformanceTierCeiling(
        DEFINING_WORLD_PLAZA_PERFORMANCE_TIER_LOW
      )
  ...
  it('allows MEDIUM starters to reach HIGH', () => {
    expect(
      resolvingWorldPlazaAdaptivePerformanceTierCeiling(
        DEFINING_WORLD_PLAZA_PERFORMANCE_TIER_MEDIUM
      )
    ).toBe(DEFINING_WORLD_PLAZA_PERFORMANCE_TIER_HIGH);
  });
});
...
describe('markingWorldPlazaAdaptivePerformanceFrame', () => {
  ...
  it('does not upgrade LOW starters past the tier ceiling', () => {
    const startedAtMs = 1_000;
    const sampler = creatingWorldPlazaAdaptivePerformanceFrameSampler(
      DEFINING_WORLD_PLAZA_PERFORMANCE_TIER_LOW,
      startedAtMs,
  ...
  it('downgrades after sustained slow p95', () => {
    ...
    expect(
      markingWorldPlazaAdaptivePerformanceFrame(sampler, slowDeltaMs, readyAtMs)
    ).toBeNull();
