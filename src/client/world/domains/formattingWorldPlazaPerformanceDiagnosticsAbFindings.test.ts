...
describe('formattingWorldPlazaPerformanceDiagnosticsAbFindings', () => {
  it('formats both captures, delta, and current snapshot', () => {
    ...
    expect(text).toContain('Current: 56.0 fps live · 55.2 fps session avg');
    expect(text).toContain('Active preset: Light');
    expect(text).toContain(
      'A: 50.0 fps live · 48.5 fps session avg · preset: Baseline'
    );
    expect(text).toContain(
      'B: 56.0 fps live · 55.2 fps session avg · preset: Light'
    );
