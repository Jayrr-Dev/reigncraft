...
function resolvingWorldPlazaPlayerNightLightWarmGlowFillGradient(): FillGradient {
  ...
  cachedWarmGlowFillGradient = new FillGradient({
    ...
    textureSpace: 'local',
    textureSize:
      DRAWING_WORLD_PLAZA_PLAYER_NIGHT_LIGHT_WARM_GLOW_GRADIENT_TEXTURE_SIZE,
    colorStops: [
      { offset: 0, color: 'rgba(255, 252, 244, 0.88)' },
      { offset: 0.18, color: 'rgba(255, 242, 214, 0.55)' },
      { offset: 0.38, color: 'rgba(255, 224, 176, 0.22)' },
      { offset: 0.58, color: 'rgba(255, 198, 132, 0.06)' },
      { offset: 0.78, color: 'rgba(255, 172, 98, 0.015)' },
...
export function drawingWorldPlazaPlayerNightLightWarmGlowOnGraphics(
  ...
  graphics.fill({
    ...
    alpha:
      glowBrightness * DEFINING_WORLD_PLAZA_PLAYER_NIGHT_LIGHT_WARM_CORE_ALPHA,
  });
}
