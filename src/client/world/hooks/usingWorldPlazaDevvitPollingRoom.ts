...
export function usingWorldPlazaDevvitPollingRoom({
  ...
  const buildingSyncPayload = useCallback((): PlazaDevvitOnlineSyncRequest | null => {
  ...
  useEffect(() => {
    ...
    updatingRoomSnapshot({
      roomIndex: 1,
      roomChannelName: "devvit-polling",
      isConnected: false,
      isJoined: false,
    ...
    const postingPlazaSync = async (): Promise<boolean> => {
      ...
      try {
        const response = await fetch(PLAZA_DEVVIT_ONLINE_SYNC_API_PATH, {
          method: "POST",
        ...
        if (!cancelled) {
          updatingRoomSnapshot({
            isConnected: false,
            isJoined: false,
            lastError: USING_WORLD_PLAZA_DEVVIT_POLLING_CONNECTION_FAILED_MESSAGE,
          });
          isJoinedRef.current = false;
        }
    ...
    const pollingRemotePlayers = async (): Promise<void> => {
      ...
      try {
        const response = await fetch(PLAZA_DEVVIT_ONLINE_PLAYERS_API_PATH);

        if (!response.ok) {
          return;
        }

        const data = (await response.json()) as PlazaDevvitOnlinePlayersResponse;
