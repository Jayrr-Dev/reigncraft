...
const PLAZA_BIOMES_GUIDE_CARD_BUTTON_CLASS_NAME =
  'group w-full cursor-pointer rounded-md border text-left transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-poster-teal/40';
...
const PLAZA_BIOMES_GUIDE_CARD_SKY_CLASS_NAME =
  'relative h-16 overflow-hidden rounded-t-[5px] sm:h-20';

const PLAZA_BIOMES_GUIDE_CARD_FOOTER_CLASS_NAME =
  'rounded-b-[5px] border-t border-poster-teal/20 px-2.5 py-2';
...
function RenderingPlazaBiomesGuideCard({
  ...
  if (!entry.isExplored) {
    return (
      <article className="relative rounded-md border border-poster-teal/20 bg-parchment/30">
        <div className="absolute right-1 top-1 z-10">
          <RenderingPlazaBiomesRarityBadge entry={entry} />
        </div>
        <div
          className={`${PLAZA_BIOMES_GUIDE_CARD_SKY_CLASS_NAME} flex items-center justify-center bg-[linear-gradient(180deg,#22333b_0%,#182329_60%,#10181d_100%)]`}
          aria-hidden
        >
        ...
        <div className="flex items-center gap-1.5 rounded-b-[5px] border-t border-poster-teal/15 px-2.5 py-2">
          <Icon
            icon="mdi:lock"
  ...
  return (
    ...
    >
      <div className="relative">
        ...
        <div
          className={cn(
            PLAZA_BIOMES_GUIDE_CARD_SKY_CLASS_NAME,
        ...
        <div className={PLAZA_BIOMES_GUIDE_CARD_FOOTER_CLASS_NAME}>
...
export function RenderingPlazaBiomesPanel({
  ...
  return (
    <div
      className={`plaza-panel plaza-pop-in flex max-h-[min(90dvh,42rem)] w-full max-w-md flex-col gap-3 overflow-visible rounded-md p-4 font-body sm:max-h-[min(85dvh,42rem)] sm:gap-4 sm:p-6 ${className}`.trim()}
    >
